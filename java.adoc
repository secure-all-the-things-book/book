...
And it's fast.
I can't give you a specific number or anything because it is so highly workload-sensitive, but https://www.optaplanner.org/blog/20{java-lts-version}/09/15/HowMuchFasterIsJava17.html[this post from the folks at OptaPlanner] is persuasive.
They saw an average improvement of 8.66% for their CPU-intensive workloads when using the G1 GC, measured after a discarded 30-second warmup period.
...
And the numbers for Java {java-lts-version}, even better still!
...
// Factor out code for boot chapter to separate git repo; add section on GraalVM to Java chapter; remove GraalVM primer from boot chapter; show AOT example in boot. Show autoconfig in boot chapter
...
This effort is ongoing, but has already born fruit.
