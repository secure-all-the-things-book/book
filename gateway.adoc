...
[source,java]
-----
include::{code}/gateway/gateway-reactive/src/main/java/com/example/gateway_reactive/SecurityConfiguration.java[]
-----
<.> you could probably get away with not defining this bean at all _if_ you were willing to deal with default behavior of CSRF tokens, which I am not.
...
<.> ...and to re-enable OAuth 2 OIDC login support and the OAuth 2 client support.
...
[source,java]
-----
include::{code}/gateway/gateway-reactive/src/main/java/com/example/gateway_reactive/GatewayConfiguration.java[]
-----
...
[source,properties]
-----
include::{code}/gateway/gateway-reactive/src/main/resources/application.properties[]
-----

<.> this process will run on port `8082`.
<.> it will use the issuer URI to validate JWTs if it detects them
<.> otherwise it will use this configuration to, acting as a client, inintiate an OIDC login to allow you to come up with a valid token.
...
[source,html]
----
include::{code}/gateway/static/index.html[]
----
...
[source,html]
----
include::{code}/gateway/static/app.js[]
----
