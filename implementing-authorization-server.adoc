...
* visit `http://127.0.0.1:8080` (important: use the IP, _not_ `localhost`!) in the browser.
* login with `josh@joshlong.com`/`password` (yes, I know it's a terrible password, don't `@` me!), and then consent when prompted.
...
[source,yaml]
-----
include::{code}/authorization-server/authorization-server/compose.yml[]
-----
...
* `josh@joshlong.com` with password `password` and roles `USER`
* `rwinch@invisible-domain.bar` with password `p@ssw0rd` and roles `USER` and `ADMIN`
