...
One interesting approach to authentication is  *HTTP BASIC*, where you send username and password (hashed) in an `Authorization` header.
...
These application servers had their very primitive facilities for authentication such that the user might've already authenticated before a request ever reached the Spring Security filter or Spring Framework code.
...
All interactions with your application's beans are gated by a sort of filter (in this case, a proxy on your bean) that checks that there's a user of some sort with a particular set of permissions These gates happen right as you make a call to a bean.
...
First thing's first: we need to _enable_ method security.
...
Imagine the kinds of tenancy checks you could build with this mind!
