...
Spring Security 7 introduced multifactor authentication, pulled in the Spring Authorization Server and Spring Security Kerberos projects, unifying them with the rest of the Spring Security efforts.
...
A well-secured system will use multiple factors for authentication, sometimes called multifactor auth or _MFA_.
...
Visit the Spring Initializr, add `PostgreSQL Driver`, `Flyway`, `JDBC API`, `GraalVM Native Support`, `Spring Web`, `Testcontainers`, `Spring Security`, `DevTools`, and `WebAuthn for Spring Security` to the build.
...
I've placed the following file in `src/main/resources/db/migration/V2__users_data.sql`.
...
Sometimes folks don't do the right thing. https://krebsonsecurity.com/2019/03/facebook-stored-hundreds-of-millions-of-user-passwords-in-plain-text-for-years/[Meta (formerly known as Facebook) was caught storing hundreds of millions of user passwords in plain text] for years in 2019. In 2019, it also came out https://www.wired.com/story/google-stored-gsuite-passwords-plaintext/[that Google had stored some passwords in plaintext] since 2005. The https://en.wikipedia.org/wiki/2012_Yahoo_Voices_hack[2012 Yahoo! Voices attack] revealed that Yahoo! had stored around 450,000 passwords in plaintext.
...
Banks generally must use multifactor authentication for customer access and sensitive transactions.
...
Users can point their MCP client of choice — Claude Code, Goose, JetBrains Air, whatever — at the MCP service and consume the content and ask questions about it as MCP resources.
Super innovative!
...
The major breakthrough came in 2022–2023 when Apple, Google, and Microsoft committed to expanding the FIDO standard to support what became known as “passkeys”—FIDO credentials that could be securely synchronized across a user’s trusted devices through cloud-based credential managers such as LastPass, Bitwarden, 1Password, iCloud Keychain, Google Password Manager, and Windows Hello.
...
* *the relying party ID* - The RP ID is the domain that a passkey is bound to.
...
* *the relying party name* - a human-readable display name.

* *allowed origins* - The allowed origins are the exact web origins that are permitted to call the WebAuthn APIs.
You need to specify _everything_: the scheme (`http` or `https`), the exact domain, and the port (if applicable).
...
== Other Interesting Kinds of Authentication

We've looked at usernames and passwords, one-time tokens, Passkeys, and we've only just begun to scratch the surface!
There are so many other ways to authenticate a user, and Spring Security supports these and many more.
I'll mention a few of them.
If you know you have these particular use cases, feel free to consult the relevant documentation for the details.

One interesting approach to authentication is *HTTP BASIC*, where you send username and password (Base64-encoded) in an `Authorization` header.

Another option is to look a user up in an *LDAP* or *Active Directory* instance, instead of a username and password database.

=== Pre-Authentication

Another interesting approach to authentication is **pre-authentication**.
In this scenario, Spring Security still needs to set up some sort of authentication and assign it to the current request, but the actual work of authenticating the user is done elsewhere.
Spring Security is instead given something it can use to understand that authentication and present it to application code as a valid `Authentication`.

If you're using an authentication and authorization platform like SiteMinder, it'll communicate a header conveying information about the authenticated user.
Spring Security can work with that through the `RequestHeaderAuthenticationFilter`.

Another approach is to use mutual *X.509* authentication.
In this scheme, clients present a certificate issued by an authority that our server knows about and trusts.

Spring Security supports *Java EE Security Services*.
In the distant, distant past, it was common to run something called an _Application Server_.
These application servers had their own very primitive facilities for authentication such that the user might've already authenticated before a request ever reached the Spring Security filter or Spring Framework code.
In such a scenario, the `J2eePreAuthenticatedProcessingFilter` class extracts the username from the `userPrincipal` property of the `HttpServletRequest`.
Use of this filter would usually be combined with the use of Java EE roles.
But... don't use any of it, if you can avoid it.
It adds nothing but complexity.
Moving on...

== Multifactor Authentication

There are tons of options here, but which is best?
Sometimes, things are so sensitive that, to be extra certain that you're talking to the right user, you want more than one factor.

Multifactor authentication is the idea that, while any single factor has weaknesses that could conceivably be exploited, it's much less likely that a hacker could exploit multiple factors, especially if they're off different categories.
We spoke earlier about the categories of authentication factors, including _knowledge_, _possession_, and _inherence_.

Spring Security 7 introduces multifactor authentication, where the system can store information about which authentication factors have been provided for a given authentication (a user) and then reconcile that against a list of outstanding factors required for that same authentication.

The simplest possible configuration is global, and annotation-powered.

[source,java,indent=0]
----
include::{code}/security/security/src/main/java/com/example/security/mfa/GlobalConditionalMfaConfiguration.java[]
----

. this configuration requires multifactor authentication for every endpoint in the system, but it's _conditional_ on the user having registered WebAuthn.
After all, they can't log in and register the Passkey if they can't log in in the first place.
If they haven't, then there's no sense in asking for it.

In the above example, the factors are combined together.
It's as though you're asking for WebAuthn _AND_ Passwords.
But that's silly.
If the user can present a passkey, then they don't really need to present a password.
Passkeys are much safer.
So, let's customize the conditions so that the user has to present one of the following:

* BOTH a password and a one-time token
* a Passkey

The specifics of multifactor auth that we've seen so far were specified as parts of the annotation, but behind the scenes those stipulations are translated onto the configuration of a global bean of type `AuthorizationManagerFactory`.
You can provide your own, if you like.
You still need to define the annotation, but you can override the specifics in the bean itself.

[source,java]
----
include::{code}/security/security/src/main/java/com/example/security/mfa/GlobalEitherOrMfaConfiguration.java[]
----

. We're going to register two required additional authorizations.
One required authorization specifies that the user must authenticate with _both_ one-time tokens and passwords.
The other just says the user must authenticate with Passkeys.

Restart the application and enter the Passkey, and you'll be staring at an authenticated page.
Logout and then enter a username or a one-time token, and you'll be asked to do another authentication.

== Authorization

Our configuration is simple and _global_.
That bean applies to every request made and to every endpoint.
Let's turn our attention to how we make more granular decisions about what is required of an authenticated user for particular aspects, like routes, of the application.
This is called _authorization_.

Spring Security supports _defense-in-depth_, allowing you to configure multiple layers - requests and methods - of security so that if for whatever reason a would be attacker breaches one layer, they can't get through the other.
Request-based authorization is the kind with which we'll mostly be dealing here.

=== Method-Level Authorization

Let's look at method-level authorization.
It can be quite useful, particularly when securing GraphQL.
It's the idea that method invocations on individual beans can be inhibited based on authorization rules.

You've probably seen the scary diagram from the Spring Security documentation.
It looks like this.

image::images/security/authorization/diagram-from-security-docs.png[]

It re-establishes some concepts we've already seen.

All interactions with your application's beans are gated by a sort of filter (in this case, a proxy on your bean) that checks that there's a user of some sort with a particular set of permissions.
These gates happen right as you make a call to a bean.
There are two kinds of checks: pre-authorization and post-authorization.
Pre-authorization checks are a good place to ensure that a given user has the right rights (permissions, roles, authorities, whatever) to make this call.
A post-authorization check is a good place to ensure that the returned value is a value to which the authenticated principal actually has rights.
Both checks throw exceptions of type `AccessDeniedException`, which is a `RuntimeException`.
You should handle this exception in your client code if you want to do something on the spot.
Otherwise, if you let it bubble up in a web container, Spring Security's default filter machinery will intercept the exception and handle it with the usual error codes.

Let's look at some examples.
First things first: we need to _enable_ method security.

TIP: You might see `@EnableGlobalMethodSecurity` in older code.
This is deprecated and you're encouraged to move to the newer approach shown here.

You can put the `@EnableMethodSecurity` on any `@Configuration` class or your `@SpringBootApplication`, which is of course a `@Configuration` class.
Of note here: Spring Boot does _not_ automatically enable method security for you by default.

[source,java]
----
include::{code}/security/security/src/main/java/com/example/security/authorization/methods/CustomerServiceConfiguration.java[]
----

Let's look at a trivial service which handles potentially sensitive data.
We want to both be sure that the person making the request is an `ADMIN` _and_ that the returned data asked for belongs to them.

[source,java]
----
include::{code}/security/security/src/main/java/com/example/security/authorization/methods/CustomerService.java[]
----

. the `@PreAuthorize` annotation's Spring Expression Language expression has access to all the methods on Spring Security's `SecurityExpressionRoot` class.
This class has convenience methods to check for all authorities, any authority, to blindly wave the attempt right on through, to reject it outright, etc.
. we've already looked up the domain data, but let's do one last check to ensure it belongs to the requestor by inspecting the method's return value.
Imagine the kinds of tenancy checks you could build with this in mind!

Now let's exercise this code.

First, we'll need a `TestcontainersConfiguration` class.
Let's look at that.

[source,java]
----
include::{code}/security/security/src/test/java/com/example/security/authorization/methods/TestcontainersConfiguration.java[]
----

Standard stuff.
You can get that generated from the Spring Initializr.

Now let's look at a test that depends on this test database and that furnishes mock users with Spring Security's `@WithMockUser`.

[source,java]
----
include::{code}/security/security/src/test/java/com/example/security/authorization/methods/CustomerServiceTest.java[]
----

. we're using Spring Boot's very convenient `@SpringBootTest` to autoconfigure the application so that we can interact with the full Spring Boot application
. Spring Security's `@WithMockUser` annotation lets us describe a contrived, _mock_, user to plug in to the context.
In this case, the user has a username of `josh@joshlong.com`, and two roles.
. this test should fail because our mock user won't have the necessary roles for authorization
. this test should fail because the returned `Customer` seemingly belongs to another user.

TIP: Spring Security provides other annotations that give you progressively more and more control over the mock object installed in the context. `@WithMockUser` lets you describe the attributes for a user which it will place in the `SecurityContext`. `@WithUserDetails` lets you specify not just the attributes of a mock `User`, but the `UserDetailsService` bean name from which to source a mock `User`.

Neat, eh?
You can use this in addition to request-based authorization, which we'll look at next.

=== Request-Based Authorization

We looked at method-based authorization because, conceptually, there are fewer moving parts.
However, most people will start with request-based authorization, wherein you secure the application as a whole, at the network protocol level, before control-flow ever enters one of your methods.
A good many of the things you want to secure are HTTP-based, and Spring Security has good authorization support for path-based matching.
But HTTP is often just the beginning; gRPC is based on HTTP, but it doesn't make sense to use HTTP path matchers for gRPC.
Ditto Model Context Protocol (MCP).

So, keeping that in mind, let's look at a trivial HTTP example, and see how we can secure - enforcing authentication and authorization - access to that application.
...
// todo add a discussion around Users
// todo add a discussion around roles vs authorities in the User object
