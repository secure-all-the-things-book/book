...
One interesting approach is to use mutual X.509 authentication.
In this scheme, clients present a certificate that's issued by an authority that our server knows about and trusts.
Another is to use HTTP BASIC, where you send username and password (hashed) in an `Authorization` header.
...
Multi-factor authentication is the idea that, while any single factor has weaknesses that could conceivably be exploited, it's much less likely that a hacker could exploit multiple factors, especially if they're off different categories.
We spoke earlier about the categories of authentication factors, including _knowledge_, _possession_, and _inherence_.

Spring Security 7 introduces multi-factor authentication, where the system can store information about which authentication factors have been provided for a given authentication (a user) and then reconcile that against a list of outstanding factors required for that same authentication.
...
. this configuration requires multi-factor authentication for every endpoint in the system, but it's _conditional_ on the user having registered WebAuthn.
After all, they can't login and register the Passkey if they don't already have the ability to login in the first place.
...
The specifics of multi-factor auth that we've seen so far were specified as parts of the annotation, but behind the scenes those stipulations are translated onto the configuration of a global bean of type `AuthorizationManagerFactory`.
...
Our configuration is simple, and _global_. That bean applies to every request made and to every endpoint. Let's our attention to how we make more granular decisions about what is required of an authenticated user for particular aspects of the application. This is called _authorization_.
