...
But first, let's look at method-level authorization. It can be quite useful, particularly when securing GraphQL.
It's the idea that method invocations on individual beans can be inhibited based on authorization rules.


Youve probably seen the scary diagram from the Spring Security documentation. It looks like this.

image::images/security/authorization/diagram-from-security-docs.png[]

Basically, Spring Security creates a proxy out of your secured bean, and intercepts method invocations, evaluating the authorization rules specified on methods (either as annotation)

// show the method authorization
// show common expressions you can put in @PreAuthorized like `isAuthenticated()`, etc.
