...
Authentication works by _challening_ the user for proof of some sort of _factor_, the demonstration of which confirms the identity of the user.
...
Examples are passwords, PINs, passphrases, secure questions, etc.
Some weaknesses of factor are that it can be guessed, phished, reused, leaked, or written down.

* *possession* - something you have.
Examples of this is a phone, hardware security key, a smart card, an authenticator app, a passkey stored onyour device, etc.
Some weakness of this factor are that the device or token can be lost, stolen, cloned in a weak system, or intercepted if it relies on text messages (SMS).

* *inherence* – something you are.
Examples of this are fingerprints, face scans, iris scans, voiceprints, etc.
Some weaknesses associated with this factor are that biometrics can be spoofed, mishandled, or impossible to _rotate_ if compromised.
...
Check the database again, and you'll see its been migrated to use the _default_ password encoder.
...
No.
Password roration used to be recommended because the assumption was that passwords could be stolen without anyone noticing.
Changing hem periodically would reduce the window during which an attacker could use them.
This made sense before the widespread adoptions of multifactor authentication, breach detection, password managers, and monitoring for compromised credentials.
But periodic rotation makes security _worse_.
Research showed the people tend to increment the number (`Passw0rd1` -> `Passw0rd2`) or make tiny, predictable modifications, or write passwords down, or choose weaker passwords because they're tired of changing them.
...
And the Revised Payment Services Directive introduced _Strong Customer Authentication_ (SCA) that requires authentication using at least two independent factors from categories such as knowledge, possession, inherence.
A password alone is not enough for many electronic payment transactions.

The *United Kingdom* has the _National Cyber Security Centre_ strongly recommends MFA for important accounts and increasingly recommends phishing-resistant authentication for government systems.
...
So far we've jsut registered beans of the right type in the right place and things have sort of worked out.

Remember when i told you that theres a giant filter sitting in between your requests and the backend application lgoic?
Well we cna configure how that works by definiing the `SecurityFilterChain` ourself, like this.
...
I show you the `SecurityFilterChain` approach because on the occasion where you want to more authorization more granular or when reading older code and older text, you'll need to appreciate what's happening.
...
E-mail, WhatsApp/SMS (though these two, and anything tied to a spoofable phone number is increasingly less encouraged these days), Discord, etc.
...
You can use this as a chance to setup any state and then look the user up and email them, call or text their phone number or WhatsApp, etc.
...
It'll tell you that you've got console mail.
Consult the console, and you'll see a link.
Copy and paste it into the browser, and you should see your username.
...
And of course, if somebody loses access to the account where they out-of-band communication is sent (email, SMS, whatever), then they're now effectively locked out of _two_ systems!

One-time tokens are an easy to way build upon the considerable infrastructure technical _and_ legal infrastructure companies like Google, Microsoft, Apple, etc., have built up to handle managing passwords and fight breaches and the like.
