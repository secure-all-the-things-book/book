...
Now let's exercise this code. While we're at it, we can take a look at some of Spring Security's amazing testing support.
...
// show the testing capabilities in CustomerServiceTest
// https://docs.spring.io/spring-security/reference/servlet/test/method.html
// do a table showing the progression from `@WithAnonymousUser1, to `@WithMockUser`, to `@WithUserDetails` to `@WithSecurityContext`. Each allowing me to exert more and more control over the 'fake' authentication that's put in place when we run the test.
