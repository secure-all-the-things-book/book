...
[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/SpringTest.java[]
----
...
There's a tension we face when doing integration tests. we want things to execute in an environment as close to production as possible - otherwise why bother with integration testing at all? - but we also need to avoid complicating test setup with unncessary dependencies.
...
Because of this, its testing support isn't about adding more configuration - it’s about removing or carving it up.
...
It depends on a `java.time.Clock` to do its work.

[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/StoreService.java[]
----
...
[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/StoreConfiguration.java[]
----
...
[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/TestBeanTest.java[]
----
...
In the last example we provided our own instance of `Clock`, which returned pre-programmed results on certain calls. for a more complex c class with more methods, this would involve surgical overrides and finagling state in the parent class.
It could get ugly really quickly. an alternative is to use Mockito, a mocking library that allows us to define objects that _look_ like our object, but whose responses (and behavior) we can provide to suit the purposes of our tests.
...
[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/MockitoBeanTest.java[]
----
...
[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/MockitoSpyBeanTest.java[]
----
