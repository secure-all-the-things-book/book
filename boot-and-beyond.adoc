...
Spring Boot will, by convention, load any `application.properties` or `application.yml` it finds in `src/main/resources`. So we no longer need our `@PropertySource` annotation.
...
We could have put this autoconfiguration class and the text file in a separate `.jar`, add that to the classpath of any Spring Boot application, and Spring Boot will give your code a chance to run _without_ requiring input from the user at all.
This is how you describe seamless, out-of-the-box, sensible defaults and configurations for your users.
...
When you work with a Spring Boot project, as generated from the Spring Initializr, you should only ever have to change one at most four versions.
...
Second, if you visit http://localhost:3000 and then navigate to `Drilldown` -> `Metrics`, you can see the same metrics you saw under `/actuator/metrics`, but now graphed over time. Neat!
...
The way GraalVM works is by starting from your `main` method entry point into the application, scanning every class that you use, and every class that those classes use, and every class that those classes use, and so forth, and onward until its built up a giant graph of which classes your program will use.
...
Remember where we talked about the Spring `BeanFactory`, and said that the `BeanFactory` handles _ingest_ (the loading of bean configuration from various sources like component scanning, Java configuration, etc), then turned everything into metamodel objects of the class  `BeanDefinition`?
