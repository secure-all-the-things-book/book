...
[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/main/java/com/example/beans_to_boot/boot/DogBeanRegistrar.java[]
----
...
But how do you know if you've actually done the right thing?
...
One of the core Spring Framework engineers, Sam Brannen, is also a major contributor to the JUnit project.
...
In the example above, we injected a _real_ `Environment`.
...
. instead of us providing a factory method, we'll tell Spring to wire up a `Mock` and replace the bean in the context with that Mock.
. then we'll use Mockito's API to instruct the `Clock` on what to do.
...
=== Endpoint Testing with `RestTestClient`

`MockMvc` gives you a servlet-centric API for driving your controllers. `RestTestClient` targets those same server-side endpoints, but through a fluent, `RestClient`-style API — the same ergonomics as `WebTestClient`, which developers on the reactive stack will recognize.
You can point it at a running server, at the whole `ApplicationContext`, or — as we do here — at a single controller, in which case there's no socket, port, or real HTTP involved at all.
Let's try it out.
...
Here we bind the client directly to the `SimpleController` and use its fluent API to issue a request against the endpoint, then make assertions about the response.
...
TIP: Spring Boot automatically pulls in a whole spate of interesting testing libraries, like Awaitility (for testing asynchronous systems), JSONPath (XPath for JSON), Mockito (a mocking library), AssertJ (for various assertions), Hamcrest (for still more assertions and matchers), and JUnit (the de facto standard testing framework).

Everything you've learned so far still applies, there's just _more_ to the story.
...
It is this latter class that we'll use to automatically stand up a PostgreSQL database process.
...
Spring Boot's got the ability to stand up your entire application and let you swap out individual components in the object graph.
This works well, but it means that you're paying the cost of constructing the entire object graph each and every time the program starts.
Spring Boot has the notion of _test slices_, which can activate only the objects you need to test a component in its particular subsystem – data, JSON, the web, security, etc.

In most cases, the user would simply add the relevant annotation from one of the dozens of supported test modules (`@JsonTest`, `@DataJdbcTest`, `@WebMvcTest`, etc.) to get the requisite autoconfiguration for just that subsystem.
Then proceed as normal.
All of these special annotations in turn are meta-annotations and use `@AutoConfigure*` variants and `@ImportAutoConfiguration`.
You can usually test with just one main annotation, e.g, `@DataJdbcTest`.

TIP: You can't have more than one main annotation, but you can have one main annotation _and_ one or more of the `@AutoConfigure*` variants.

Let's look at an example.

[source,java]
----
include::{code}/beans-to-boot/beans-to-boot/src/test/java/com/example/beans_to_boot/boot/DataJdbcTestTest.java[]
----

the `@DataJdbcTest` starts an embedded SQL database for us.
Recall that when we started this project we added `H2` to the specification on the Spring Initializr.
Likewise, it runs all tests in a transaction that is automatically rolled back once the test is done, ensuring that the database state is consistent between tests.
You get all this _while_ writing less code and while executing a bit faster than when running the full Spring `ApplicationContext`.
These gains pay off in a larger suite of tests.

All in all, Spring Boot's testing support is a crucial way to think about how to build software.
